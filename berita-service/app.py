...
def index():
  ...
  cursor.execute("SELECT berita.title, berita.author, category.category, berita.isi_berita FROM berita JOIN category ON berita.id_berita = category.id_category")
...
def create():
  ...
  cursor = db.cursor(dictionary=True)
  query = 'INSERT INTO berita (id_berita, title, kategori, author, isi_berita) VALUES ("%s", "%s", "%s", "%s", "%s")' % (berita['id_berita'], berita['title'], berita['kategori'], berita['author'], berita['isi_berita'])
